  ...
  return (
    <main className="auth-new">
      <div className="auth-new-aside">
        <span className="app-kicker">Account recovery</span>
        <h1>Choose a new password.</h1>
        <p>
          Use the token from your reset email. Passwords are never stored in the
          browser.
        </p>
      </div>
      <form
        className="auth-new-card"
      ...
      >
        <span className="app-kicker">Reset access</span>
        <Input label="Reset token" value={token} onChange={setToken} />
        <Input
          label="New password"
          value={password}
...
export function PurchaseCourse({ courseId }: { courseId: string }) {
  const resource = useData<RecordValue>(`/learning/courses/${courseId}`);
  const course = resource.data;
  return (
    <Shell section="Course purchase">
      <PageHeader
      ...
      />
      <Panel>
        <span className="app-kicker">What you receive</span>
...
export function PeopleContract() {
  ...
  return (
    <Shell section="People">
      <PageHeader
        ...
        description="Create accounts with the complete admin user contract."
      />
      <Panel>
        <form
  ...
  return (
    <Shell section="Themes" admin>
      <PageHeader
        ...
        title="Theme library."
        description="Theme records include the complete token JSON required by the backend."
      />
      <Panel>
        <form
      ...
      <div className="spacer-18" />
      <DataTable
        columns={['Theme', 'Description', 'Status', 'Edit']}
      ...
      {selected && (
        <Panel>
          <span className="app-kicker">Edit theme</span>
          <Button
            onClick={() =>
              patch(`/tenant/themes/${selected.id}`, {
                name: selected.name,
                description: selected.description,
                tokenJson: selected.tokenJson || {},
              }).then(() => {
                setSelected(null);
                resource.reload();
              })
