    ...
    try {
      ...
      if (login) {
        ...
        payload = {
          ...
          tenantSlug,
          ...(showMfa && values.mfaCode ? { mfaCode: values.mfaCode } : {}),
        };
        ...
        const workspace = slugify(values.tenantSlug || values.tenantName || '');
        payload = {
          firstName: values.firstName || '',
          lastName: values.lastName || '',
          email: values.email || '',
          ...
          subdomain: values.subdomain || workspace,
        };
      } else {
        path = '/auth/password-reset/request';
        payload = {
          email: values.email || '',
          tenantSlug: values.tenantSlug || tenantSlug,
        };
      }
      ...
      if (login && message.toLowerCase().includes('mfa')) setShowMfa(true);
      setError(message);
    } finally {
      setBusy(false);
    }
      ...
      <form className="auth-new-card" onSubmit={submit}>
        ...
        <p className="auth-subtitle">
          {login
            ...
            : reset
              ...
              : instructor
                ? 'Set up your branded creator workspace.'
        ...
        {instructor && (
          <>
            ...
            <Field
              ...
              onChange={(value) => set('tenantSlug', slugify(value))}
              placeholder="your-practice"
            />
            <Field
              label="Subdomain"
              value={values.subdomain || ''}
              onChange={(value) => set('subdomain', slugify(value))}
              placeholder="your-practice"
            />
          </>
        )}
        ...
        {login && showMfa && (
          <Field
            ...
            onChange={(value) => set('mfaCode', value)}
            placeholder="123456"
