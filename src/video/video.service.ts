...
export class VideoService {
  ...
  constructor() {
    ...
    this.drm =
      process.env.DRM_PROVIDER === 'simple'
        ? new SimpleDrmAdapter()
        : new SimpleDrmAdapter();
  }

  // DRM-aware playback signing
  signPlayback(videoId: string, userId?: string) {
    ...
    return {
      signedUrl: `https://cdn.example/videos/${videoId}/master.m3u8?sig=stub&user=${userId ?? 'anonymous'}`,
      drm: false,
    };
  }

  requestLicense(videoId: string, drmType: string, clientData: unknown) {
    return this.drm
      .requestLicense(videoId, drmType, clientData)
      .catch((err) => {
        this.logger.warn('DRM adapter failed, returning error token', err);
