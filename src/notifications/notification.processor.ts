...
export class NotificationProcessor implements OnModuleInit, OnModuleDestroy {
  ...
  private readonly pollIntervalMs = Number(
    process.env.EMAIL_PROCESS_INTERVAL_MS ?? '30000',
  );
  ...
  onModuleInit() {
    this.logger.log(
      `Starting notification processor every ${this.pollIntervalMs}ms`,
    );
    this.interval = setInterval(() => {
      void (async () => {
        try {
          ...
          this.logger.warn('Notification processor failed', error);
        }
      })();
    }, this.pollIntervalMs);
  }
